...
- `sdk.security.permit-all-paths` uses `WebSecurityCustomizer.ignoring`, which bypasses the security
  filter chain. The canonical list must be owned and changed only in `sdk_util`; SDK consumers must
  not duplicate or override it in application YAML.
- Keep the centralized list least-privilege and limited to technical or explicitly approved shared
  paths. Never add broad business patterns such as `/api/v1/**`.
- A centralized-list change affects every SDK consumer and requires compatibility notes plus
  focused security tests for authenticated, unauthenticated, and permitted requests.
- Do not assume that an `internal` path name provides a security boundary. If `/internal/**` is
  centrally permitted, isolate it from public ingress using network policy, firewall, or service
  mesh in every deployment.
